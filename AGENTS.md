...
- スタック: React 19 + Vite 7 + Tailwind CSS v4（`@tailwindcss/vite` プラグイン、`src/index.css` の `@theme` でトークン定義）+ Firebase（Auth/Firestore/Functions v2）+ Anthropic API
- AI 呼び出しはすべて `functions/src/`（Cloud Functions）経由。クライアントに API キーを置かない
- モデル: 対話 `claude-sonnet-4-6`（SSE ストリーミング）、軽量タスク `claude-haiku-4-5`（`output_config.format` の JSON Schema 構造化出力）
